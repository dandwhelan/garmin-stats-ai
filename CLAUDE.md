...
| `garmin-insights/src/garmin_insights/scales/` | Server-side Bluetooth-scale decoding + body-composition maths — the browser is a dumb BLE pipe (discover, handshake, relay notify frames as hex); everything that can be wrong is decided and unit-tested here. `base.py` (`AdapterDescriptor`, `ScaleReading`, `ScaleAdapter` protocol, `parse_hex_frames` — tolerant hex decoding that never raises on untrusted browser input), `lefu.py` (`LefuAdapter` for the Fitdays/Lefu 8-electrode scale family, vendor GATT service `0xFFB0` — auto-detects two on-the-wire layouts, `lefu_ac02` and the E.volve/ESPHome `lefu_offset`, plus a conservative impedance-frame decode), `registry.py` (`ADAPTERS` dict + `get_adapter`/`descriptors` — the one-line spot to register a new scale family), `composition.py` (`compute_body_composition` — the impedance-based BMI/fat/water/muscle/bone/visceral/metabolic-age/fat-mass/fat-free-mass/BMR formulas, ported verbatim from the old client-side JS; formulas trace to lswiderski/WebBodyComposition ← wiecosystem/Bluetooth). **⚠ The `lefu.py` decode is known-wrong and not yet fixed — see `docs/scale-protocol/`:** weight is a BE u24 at `frame[7:10]` in grams, not `frame[8:10]/100` (the current code drops the high byte and yields 501.50 kg in the same session as 72 kg); the composition frame arrives on **FFB3 (indicate)**, which the adapter never subscribes to; and `_decode_impedance` gates on `frame[0] == 0xFF`, which can never match because `frame[0]` is the sequence byte. The protocol is fully documented and the frame checksum cracked in `docs/scale-protocol/README.md`; the specified fix is track (C) of `docs/scale-protocol/NEXT-STEPS.md`. The vendor's own composition model (`WLA37`, from `libICBodyFatAlgorithms.so` in the Fitdays APK) reproduces the app's figures from the 10 segmental impedances — see README §11/§11.7; it runs only under Unicorn emulation on Raspberry Pi OS, and redistributing that proprietary binary is an open licensing question. |
