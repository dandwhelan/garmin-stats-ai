...
- **Journal tab** — write a free-text note for any day (what you did, ate, how you felt). Saved via `POST /api/notes`; an empty body clears the day. Notes are read back into the editor + a "recent notes" list (`GET /api/notes`) and merged into the AI's daily summaries so the agent treats them as first-hand ground truth when explaining metric deviations. A **navigable month calendar** sits above the recent-notes list: dots mark days that already have a note, clicking a day loads it into the editor, and prev/next walk between months. Notes fetches use `cache: 'no-store'` so a freshly-saved note shows immediately. Below the note editor, a **Log a Weigh-in** form (weight + optional smart-scale fields) uploads a body-composition entry straight to Garmin Connect via `POST /api/weigh-in` — a local replacement for lswiderski's mi-scale-exporter web form.
