...
### MCP server (per-user, for Cursor / Claude Desktop / Antigravity)
```bash
pip install -e "garmin-insights[mcp]"
garmin-insights-mcp --user dan            # stdio
# Always-on on the Pi: garmin-insights-mcp@dan (:8765) / @helen (:8766)
sudo systemctl restart garmin-insights-mcp@dan garmin-insights-mcp@helen   # after any Python change
```
...
| File | Purpose |
|------|---------|
| `garmin-insights/src/garmin_insights/agent.py` | Core Claude agent — tool-calling loop, prompt caching, streaming, per-model thinking config. Dynamic system blocks (`_identity_block`, `_cycle_context_block`, `_environment_context_block`, `_evidence_tier_block`) inject the active user's name + biological sex, current cycle phase (for menstruating users), today's environmental extremes (heat / poor AQ / high pollen, when above thresholds), and the evidence-tier output rules that govern how confidently the model phrases findings. The portable prompt merges per-day environment fields (`env_*`) directly into the daily summaries dict so paste-into-ChatGPT inherits the same context. `generate_scan_report` self-injects the deterministic `InsightScanner` findings when the caller passes no context (so web scans, the scheduler and chat `/scan` lead with code-computed anomalies instead of re-deriving them — skipped for explicit historical ranges, whose windows would contradict the today-anchored scanner), raises reasoning effort to a per-focus floor (`_SCAN_MIN_EFFORT`: weekly/general → medium; never lowers a higher configured effort), and persists every successful report via `MemoryStore.save_scan_report`. Scan foci: morning / midday / evening / night / general / weekly. The weekly scan checks `get_experiments` for active personal experiments and reports each one's compliance + interim effect (silently skipped when there are none); the `_SYSTEM_PROMPT` "Personal experiments" rule tells the agent to offer `start_experiment` when the user states a testable behavior→metric hypothesis and to frame verdicts as the user's own n-of-1 evidence, not clinical proof. |
| `garmin-insights/src/garmin_insights/tools/query_tools.py` | 35 tool definitions (Anthropic JSON schema) + handler methods. Token-efficiency helpers: `_round_floats` (1 d.p.), `_clean_records` (strips nulls + rounds), `_strip_zero_lifestyle` (drops zero-status entries, compacts to `["Behavior: N"]` strings), `_truncate_long_lists` (caps chronological lists, keeps the tail). `get_daily_metrics` returns a date-keyed dict `{"YYYY-MM-DD": {...}}` (not an array) to remove repeated `"date"` fields. `get_my_baselines` strips null sub-fields. `get_environment_data` exposes Open-Meteo weather/AQ/pollen so the agent can quantify environmental confounders for RHR/HRV/respiration/sleep deviations. `get_fitness_markers` surfaces the slow-moving VO2 max / fitness age / race predictions / endurance / hill score (both date args optional, year-long default look-back). `get_activity_history` includes hr_time_in_zone_1..5, speed, power and running dynamics (nulls stripped). Bridging tools reuse dashboard/local analytics in chat: `get_training_status` (load / ACWR / fitness trend, latest per day, 2 d.p.), `get_intraday_data` (hourly digest + peak/low of intraday HR / stress / body battery / steps / respiration for one local day), `get_lifestyle_analytics` (16 whitelisted LifestyleService analytics), `get_behavior_root_cause` (per-event 48h confounder scan), `get_bedroom_environment` (HA sensors), `get_environment_forecast` (planning-only Open-Meteo forecast — clearly labelled, never for explaining past metrics), `scan_all_anomalies` (one-call anomaly sweep over every baselined metric). The **personal experiment engine** adds three n-of-1 tools: `start_experiment` (register a testable behavior→metric hypothesis, validates the metric + fuzzily resolves the behavior, warns when it isn't logged yet), `get_experiments` (list active/completed/abandoned with interim results), and `evaluate_experiment` (Welch t-test + Cohen's d on behavior-days vs non-behavior-days since the start date, via `AnalysisEngine.evaluate_experiment` reusing the date-based lag join — returns a verdict with honest n-of-1 caveats; `conclude`/`abandon` flags close it out). The tool-list cache marker is applied programmatically to the LAST definition so appending tools can't strand it mid-list. `get_scale_readings` (34th tool) surfaces locally-recorded Bluetooth-scale readings — weight + bio-impedance-estimate composition fields, flagging `adapter="manual"` rows as hand-entered rather than scanned. `get_overnight_physiology` (35th) returns the per-night SHAPE of the overnight series (see `insights/overnight.py`) — the shape the nightly averages hide. |
| `garmin-insights/src/garmin_insights/scales/` | Server-side Bluetooth-scale decoding + body-composition maths — the browser is a dumb BLE pipe (discover, handshake, relay notify frames as hex); everything that can be wrong is decided and unit-tested here. `base.py` (`AdapterDescriptor`, `ScaleReading`, `ScaleAdapter` protocol, `parse_hex_frames` — tolerant hex decoding that never raises on untrusted browser input), `fitdays.py` (`FitdaysAdapter`, the **default** adapter — the full-BIA protocol of the Fitdays/Lefu `JEETIF` 8-electrode scale: frame codec + 5-bit checksum, `A2` live weight (BE u24 grams), `A7` result with ten segmental impedances, and `plan_writes()`, which decides the handshake from what the scale has sent so far since it needs a fresh timestamp and must arrive while the user is standing on the scale; a result is only accepted after an `A2` status-`03` settle, because the scale replays its previous result on connect; protocol documented publicly at `dandwhelan/fitdays-lefu-scale`), `wla37.py` (`compute_wla37` — Fitdays' own body-composition model, run from the proprietary `libICBodyFatAlgorithms.so` under Unicorn emulation; the `.so` is **never committed** and is looked up at `WLA37_SO_PATH` or `<repo>/vendor/` (gitignored), with optional deps `garmin-insights[scale-engine]`; returns `None` when unavailable so the scan falls back to `composition.py`), `lefu.py` (`LefuAdapter` — older weight-only Lefu layouts `lefu_ac02` / `lefu_offset`, kept for other units), `registry.py` (`ADAPTERS` dict + `get_adapter`/`descriptors` — the one-line spot to register a new scale family), `composition.py` (`compute_body_composition` — the impedance-based BMI/fat/water/muscle/bone/visceral/metabolic-age/fat-mass/fat-free-mass/BMR formulas, ported verbatim from the old client-side JS; formulas trace to lswiderski/WebBodyComposition ← wiecosystem/Bluetooth). |
...
| `garmin-insights/src/garmin_insights/mcp_server.py` | MCP server — one process per user (`--user`), named `garmin-insights-<user>`. `AgentContext` holds the same data stack as `HealthAgent` minus the Anthropic client, so the agent's prompt-block methods are called unbound (`HealthAgent._identity_block(ctx)` etc.) and stay identical: the static system prompt (base rules + KB + evidence-tier rules + identity) becomes the MCP instructions; the day-varying blocks + `_local_scan_context` findings are served per call by the `get_current_context` tool / `garmin://context/today` resource and prepended to every scan prompt. `get_evidence(rule)` / `garmin://knowledge/{rule}` return one KB rule's citation, tier, confounders (sex-filtered). Write tools are hidden unless `--allow-writes`. systemd template `deploy/garmin-insights-mcp@.service`, ports in `deploy/mcp-<user>.env`. |
| `garmin-insights/src/garmin_insights/web/app.py` | FastAPI server — SSE chat, dashboard (auto cache-refresh + date params + cycle-field enrichment), scan endpoints (with optional date range), user/sync identity, `/api/visualizations`, `/api/lifestyle`, `/api/intraday/heatmap`, `/api/menstrual`, `/api/environment`, `/api/environment/recovery`, `/api/activities/{id}/export`, `/api/scale/adapters`, `/api/scale/frames`, `/api/scale-readings`. The Bluetooth-scale flow keeps an in-memory `_scale_sessions` buffer keyed by `(user, session_id)`, bounded (`_SCALE_SESSION_MAX_FRAMES`, `_SCALE_SESSION_MAX_LIVE`, `_SCALE_SESSION_TTL`) and evicted (`_evict_scale_sessions`) ahead of every request since the input is an untrusted browser relaying an unknown-firmware scale's raw frames. |
...
| `garmin-insights/src/garmin_insights/knowledge/medical.py` | 57 evidence-tier-graded insight rules (`InsightRule` with `evidence_tier`, `claim_strength`, `measurement_confidence`, `confounders`, `requires_user_context`). Includes the `multi_cause_recovery_strain` meta-rule, `baseline_reliability_guard`, `travel_circadian_disruption`, an environmental cluster (`heat_recovery_confounder`, `air_quality_recovery_confounder`, `high_pollen_sleep_confounder`, `allergy_next_day_rhr_systemic` per Buekers 2023, `asthma_environmental_hr_marker` per Cokorudy 2024), and an overnight-physiology cluster keyed to `insights/overnight.py` rather than the daily cache (`blunted_nocturnal_hr_fall` per Ben-Dov 2007, `delayed_overnight_hr_nadir` per Pietilä 2018, `overnight_desaturation_burden` per Kapur 2017 + Khosla 2018, `overnight_hrv_trajectory` per Boudreau 2013, `sleep_fragmentation_waso` per Ohayon 2017). |
| `garmin-insights/src/garmin_insights/insights/proactive.py` | `InsightScanner` — local anomaly + behavior + trend detection, enriched with tier metadata. `scan_composite_strain()` collapses concurrent strain-direction anomalies (RHR↑ / HRV↓ / respiration↑ only — deviations the "good" way never trigger it) into a single ranked-contributor finding. `scan_overnight()` adds the overnight-physiology deviations and screening labels, so they ride into `generate_scan_report`'s injected context, the portable prompt's precomputed findings, and the CLI `scan` from one pass. |
| `garmin-insights/src/garmin_insights/insights/overnight.py` | `OvernightService` — per-night physiology from the `sleep_intraday` per-sample series, which every other analytic ignores in favour of the one-row-per-night summary. Computes the **shape** of a night the averages hide: `hr_dip_pct` (fall from sleep-onset HR to the overnight low) and `hr_nadir_pct_of_night` (how far through the night the low arrived — a small fall with a late trough is the alcohol / late-meal / late-training signature), the HRV trajectory (`hrv_first_third` vs `hrv_last_third`), the SpO2 **desaturation burden** (`desat_index_per_hour` — device-estimated from sparse wrist pulse-ox, explicitly **not** a clinical ODI and never a sleep-apnoea finding), respiration CV, body-battery recharge rate, and stage fragmentation / `waso_minutes`. Baselines are the user's own nights (robust median/MAD); deviations fire only in each metric's concern direction (`CONCERN_DIRECTIONS`, mirroring the proactive scanner) so an unusually good night is never narrated as a worry. Nights are split at **local noon** to stay aligned with `sleep_summary`'s wake-date keying, converted per timestamp so a window spanning a DST change carries both offsets. Results are cached per night in `overnight_cache`, invalidated by sample count via an index-only census — the two newest nights always recompute. |
| `garmin-insights/src/garmin_insights/web/static/` | Frontend: `index.html`, `style.css`, `app.js` (date range toolbar, customize panel, info-icon tooltips, user/sync badges, Entities tab, ~18 secondary chart renderers). The Bluetooth-scale scan (`scanScale`) is a dumb BLE pipe — it fetches the adapter descriptor from `GET /api/scale/adapters`, connects, subscribes to the descriptor's notify **and** `indicate_uuid` characteristics, and relays every frame to `POST /api/scale/frames` as hex (for `server_writes` adapters such as Fitdays there is no fixed handshake — it writes the frames returned in each response's `writes` list, strictly in order ~150 ms apart via a promise queue); it contains no protocol constants and no body-composition maths (both live server-side in `scales/`). An optional **debug capture** checkbox timestamps and logs every handshake/keepalive write (↑) and notification (↓) as hex to a capped 500-entry `<pre>`, copyable via a "Copy raw log" button — for diagnosing an unmatched Lefu firmware layout. |
...
- `body_composition` — weight, BMI, body fat/water %, bone/muscle mass, visceral fat, metabolic age. The latest reading per day is merged into `daily_summaries` (`weight_kg`, `bmi`, `body_fat_pct`, `body_water_pct`, `muscle_mass_kg`, `bone_mass_kg`, `visceral_fat`, `metabolic_age` — masses normalised to kg via `stats_utils.to_kg`, metabolic age from Garmin's ms duration to years via `stats_utils.metabolic_age_years`) and `weight_kg`/`body_fat_pct`/`visceral_fat` are baselined, so trend/anomaly/correlation tools, the Entities picker and the portable prompt all see scale data, not just the raw `get_body_composition` tool
...
- `sleep_intraday` — overnight per-sample series (SpO2, respiration, sleep HR, stress, body battery, HRV, sleep stage, movement, restlessness). Populated going forward by the fetcher's `SleepIntraday` handler (a COALESCE merge-upsert — sample types share `(time, device)` and must not clobber each other); historical nights from before the handler existed are absent. Read by `insights/overnight.py` — columns are `spo2_reading` / `respiration_value` / `heart_rate` / `stress_value` / `body_battery` / `hrv_value` / `stage_level` / `stage_seconds` / `activity_level` / `restless_value` (Garmin writes `-1` for "no reading"; `stage_level` is 0 deep / 1 light / 2 REM / 3 awake).
- `overnight_cache` — derived per-night metrics keyed by `night_of`, with the `samples` count that invalidates them. Written by `OvernightService`; safe to delete (it rebuilds).
- `scan_reports` — persisted AI scan narratives (focus, report markdown, optional date range). Written by `generate_scan_report` on every successful scan; listed via `GET /api/scans`.
- `daily_summaries` — pre-computed cache used by the LLM (faster than raw queries). Carries the camelCase subsets in `sqlite_repo._DAILY_STATS_COLS` / `_SLEEP_COLS` — incl. `sedentarySeconds`, `bodyBatteryDuringSleep` and overnight `lowestSpo2Value`, which the `sedentary_stress_coupling` / `overnight_spo2_disordered_breathing` rules need (both are also baselined)
...
- `experiments` — n-of-1 personal behavior trials (personal experiment engine). One row per hypothesis: `name` (unique), `hypothesis`, `behavior` (the lifestyle-journal entry to split on), `metric` (target daily metric), `expected_direction`, `lag_days` (behavior→metric alignment, default 1), `min_days_per_arm`, `status` (active/completed/abandoned), `verdict`, `result_json` (last evaluation) and `started_at`/`ended_at` (space-separated UTC). Managed by `MemoryStore.create/get/record_result/conclude/abandon_experiment` and the `start_experiment`/`get_experiments`/`evaluate_experiment` tools
- `scale_readings` — one row per Bluetooth-scale scan or manual weigh-in: `taken_at`/`date`, `adapter` (`lefu` or `manual`), `weight_kg`, `impedance_ohm`, the composition fields (`bmi`, `body_fat_pct`, `body_water_pct`, `muscle_mass_kg`, `bone_mass_kg`, `visceral_fat`, `metabolic_age`), `extras_json` (fat_mass_kg/fat_free_mass_kg/bmr_kcal + adapter-specific diagnostics like impedance candidates), `raw_frames` (hex, never returned by the API), and `uploaded_to_garmin`. Written by `POST /api/scale/frames` (a completed scan) and `POST /api/weigh-in` (a manual entry, or to mark an existing scan-derived row uploaded via `reading_id`). Read via `MemoryStore.get_scale_readings`/`get_latest_scale_reading`, the `GET /api/scale-readings` endpoint, and the `get_scale_readings` agent tool

## Web UI Features

- **Journal tab** — write a free-text note for any day (what you did, ate, how you felt). Saved via `POST /api/notes`; an empty body clears the day. Notes are read back into the editor + a "recent notes" list (`GET /api/notes`) and merged into the AI's daily summaries so the agent treats them as first-hand ground truth when explaining metric deviations. A **navigable month calendar** sits above the recent-notes list: dots mark days that already have a note, clicking a day loads it into the editor, and prev/next walk between months. Notes fetches use `cache: 'no-store'` so a freshly-saved note shows immediately. Below the note editor, a **Log a Weigh-in** form (weight + optional smart-scale fields) uploads a body-composition entry straight to Garmin Connect via `POST /api/weigh-in` — a local replacement for lswiderski's mi-scale-exporter web form. A **📶 Scan scale** button reads the user's Fitdays/Lefu 8-electrode scale (vendor GATT service `0xFFB0`) over Web Bluetooth: the browser is a **dumb BLE pipe** — it fetches the adapter descriptor from `GET /api/scale/adapters`, connects, subscribes to its notify and indicate characteristics, streams every raw frame as hex to `POST /api/scale/frames`, and writes whatever handshake frames the server returns (the Fitdays protocol needs a fresh timestamp and must be sent while the user is already standing on the scale, so the server plans it). A settled scan fills the form with Fitdays' own figures (via `scales/wla37.py`) and saves the per-limb breakdown locally, since Garmin Connect has no fields for it. All GATT UUIDs, handshake bytes, frame-layout knowledge and the BIA body-composition maths (fat/water/muscle/bone/visceral/metabolic-age/fat-mass/fat-free-mass/BMR) live server-side in `garmin_insights/scales/`, computed from the user's `HEIGHT_CM`/`BIRTH_DATE`/`BIOLOGICAL_SEX` profile — the height/age/sex fields still shown in the UI are the user-visible record of what that calculation assumed, not inputs to any client-side formula. A **debug capture** checkbox logs every frame (direction + hex + timestamp) to a copyable panel, for diagnosing which of the (at least three) known Lefu FFB0 firmware layouts a given unit speaks. `reading_id` from a completed scan is threaded through the upload so it updates that scan's local `scale_readings` row instead of creating a duplicate. **Web Bluetooth needs a secure context** — over plain `http://pi5:8081` the button explains the options (tailscale serve HTTPS, localhost, or the chrome insecure-origin flag).
...
- **Dashboard chart catalogue** (~26 sections total). Every chart tooltip carries an evidence-tier chip (`[Tier A/B/C]`) so the user can see how strong the underlying research is:
  ...
  - Overnight Physiology (auto-hidden per chart when the window has no per-sample series): **Overnight Heart-Rate Fall** (Tier B, medium-confidence — fall from sleep onset to the overnight low, plus how far through the night it arrived; Ben-Dov 2007, Pietilä 2018), **Overnight Desaturation & Fragmentation** (Tier C, low-confidence measurement — device-estimated SpO2 drops per hour, explicitly not a clinical ODI, against WASO minutes; Kapur 2017, Khosla 2018, Ohayon 2017)
  - Lifestyle & Health Insights: **Illness-like Recovery Strain Pattern** (Tier B, Quer 2021 + Lancet Digital Health 2022 SR — explicitly non-diagnostic), Recovery Debt, Inflammation Index (Tier C — composite physiological-strain z-score, not a measurement of inflammatory biomarkers), SRI (Tier A, Windred 2024), Social Jet Lag dual-clock (Tier B), Stress Resilience, Body Battery Decay, Behavior Recovery Cost, Dose-Response (per-behavior picker; rendered as **small multiples** — one mini-chart per metric (sleep / HRV / deep sleep / RHR) on its own scale, showing raw nights + a bold mean-per-dose line, since the four metrics share no common axis), Caffeine Cutoff (Tier A, Drake 2013 + 2023 meta-analysis), Habit Half-Life, Streak Calendar, Co-occurrence Matrix, Stress Trigger Leaderboard, Stress Hour-of-Day Fingerprint, Step-Count CDF (Tier A, Paluch 2022), WHO Activity Target (Tier A, Bull 2020), VO2 Max / Fitness Age (Tier A, Lang 2024 BJSM overview, >20M observations), **Fitness Trajectory** (Tier A — VO2 max run/cycle trend + modelled 5k/10k/half/marathon race times + endurance & hill score; auto-extends the look-back to ~180 days since these markers move slowly; auto-hidden when the device reports none), **Scale Detail** (Tier B, medium-confidence measurement — fat mass / fat-free mass / BMR from Bluetooth scale scans, decoded server-side; bio-impedance estimates, not lab measurements; auto-hidden until a scan produces a reading), **Segmental Muscle** and **Segmental Fat** (Tier B — per-limb % of standard for trunk / both arms / both legs from Fitdays' model; local-only history since Garmin Connect has no per-limb fields; segmental fat is an inferred value; auto-hidden until a Fitdays scan exists)
...
| `GET /api/overnight?start=&end=` | Per-night overnight physiology from `sleep_intraday`: HR fall + nadir timing, HRV trajectory, SpO2 desaturation burden, respiration CV, body-battery recharge rate, stage fragmentation / WASO — plus the user's own robust baselines, the latest night's deviations, and any screening labels. `available: false` for databases written before the fetcher began recording the overnight series. | last 30 days |
...
| `POST /api/scan` (body: `focus`, optional `start_date`, `end_date`) | Markdown AI report scoped to focus + optional window. Foci: general / morning / midday / evening / night / weekly. Reports are persisted to `scan_reports`. | focus-dependent |
| `GET /api/scans?user=&limit=&focus=` | Persisted AI scan reports, newest first (max 50) | last 10 |
| `GET /api/scale/adapters` | Protocol descriptors for every known Bluetooth-scale family (`garmin_insights/scales/registry.py`) — GATT service/notify/write UUIDs, handshake frames, keepalive poll, name-prefix fallbacks. No user context or DB access; tells the browser what to write and where. | — |
| `POST /api/scale/frames` (body: `user`, `session_id`, `adapter`, `frames` — raw BLE notification payloads as hex) | Folds one batch of a scan's frames into a running decode via the named adapter. Every response carries `writes` (hex frames the browser must write next — the Fitdays handshake once live weight shows a person on the scale, then the result acknowledgement), planned from per-session state and built from the user's `HEIGHT_CM`/`BIOLOGICAL_SEX`/`DISPLAY_NAME`. A settled Fitdays result runs `compute_wla37` and stores the vendor figures in the columns plus `extras` (`segments` per-limb fat/muscle %, `impedance_segments_ohm`, `bmr_kcal`, `protein_pct`, `skeletal_muscle_pct`, `subcutaneous_fat_pct`, `fat_mass_kg`, `fat_free_mass_kg`, `body_score`); without the vendor engine it still saves weight + raw impedance and notes why. Untrusted input (unknown-firmware scale, flaky connection) — sessions are buffered server-side, capped, and TTL-evicted. Returns `{state: "waiting"\|"reading"\|"final", ...}`; a `"final"` response includes `reading_id`, `weight_kg`, `impedance_ohm`, computed `metrics`, and persists the reading to `scale_readings`. | — |
| `GET /api/scale-readings?user=&start=&end=` | Locally-persisted scale readings for the window (never includes `raw_frames`). Backs the Scale Detail chart and the `get_scale_readings` agent tool. | last 90 days |
| `POST /api/weigh-in` (body: `user`, `weight_kg` + optional `timestamp`, `bmi`, `body_fat_pct`, `body_water_pct`, `muscle_mass_kg`, `bone_mass_kg`, `visceral_fat`, `metabolic_age`, `physique_rating`, `basal_met_kcal`, `reading_id`) | Uploads a body-composition entry to **Garmin Connect** via `garmin_upload.py`. `basal_met_kcal` is the BMR the Fitdays scan computes, sent as Garmin's `basal_met`. `physique_rating` is deliberately **never** filled from a scan: the vendor's `body_type` enum (0-8) tracks body fat (9.4% fat -> 2, 47-60% fat -> 8), whereas Garmin's 1-9 rating is a Tanita-style code where 9 means *very muscular* — so `body_type + 1` would record the fattest reading as the most muscular. `body_type` is surfaced in the scan readout for display only. Garmin also accepts `active_met` and `visceral_fat_mass`, which this scale does not report (its visceral figure is a rating, not a mass), so those stay unset rather than being synthesised, using the OAuth tokens the fetcher maintains in the user's `TOKEN_DIR` (token-only login; verifies token ownership against the user's email before uploading). `reading_id` (from a prior `/api/scale/frames` scan) marks that existing `scale_readings` row uploaded instead of creating a duplicate; omitted/`null` for a manual entry, which creates a new row. The reading flows back into `body_composition` on the next fetch. Form lives in the Journal tab ("Log a Weigh-in"). | — |
...
- The medical knowledge base (`knowledge/medical.py`) contains **57 evidence-tier-graded insight rules** (13 Tier A, 31 Tier B, 13 Tier C) injected into the system prompt (~13.4k chars after optimisation — covers sleep, lifestyle, recovery, training load, illness-like patterns, body composition, menstrual cycle, travel, environmental confounders (heat / air quality / pollen + Buekers 2023 systemic allergy + Cokorudy 2024 asthma-marker), and a meta-rule for multi-cause attribution)
...
- **Cumulative metrics exclude today**: `analysis_tools._CUMULATIVE_METRICS` (steps, sedentarySeconds, stress %, …) are filtered to complete days in anomaly / trend / comparison paths — a partial day's running total otherwise reads as a huge negative z-score.
- **Unrecorded nights**: when a complete day has < 1 h of `sleepingSeconds` (watch off overnight), `CacheBuilder.build_daily_summary` drops both `sleepingSeconds` and `sedentarySeconds` — Garmin books the unrecorded night as ~21 h sedentary, which would inflate the baseline.
- **Journal activity window**: Garmin writes a status-0 row for every tracked behaviour every day, so "logged no" ≡ "didn't log". `compare_metric_with_behavior` and the dashboard's `behavior_impact` count a day in the *without* arm only if it is within ±3 days of any logged-present behaviour (`journal_active_dates`), so days after a user stops journaling don't bias comparisons. `evaluate_experiment` deliberately skips this gate (its protocol has off-days left unlogged). Scan context injects only FDR-significant behaviour impacts.
