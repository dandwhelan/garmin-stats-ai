...
| `garmin-insights/src/garmin_insights/web/static/` | Frontend: `index.html`, `style.css`, `app.js` (date range toolbar, customize panel, info-icon tooltips, user/sync badges, Entities tab, ~18 secondary chart renderers). The Bluetooth-scale scan (`scanScale`) is a dumb BLE pipe — it fetches the adapter descriptor from `GET /api/scale/adapters`, connects, runs the handshake, and relays every notification frame to `POST /api/scale/frames` as hex; it contains no protocol constants and no body-composition maths (both live server-side in `scales/`). An optional **debug capture** checkbox timestamps and logs every handshake/keepalive write (↑) and notification (↓) as hex to a capped 500-entry `<pre>`, copyable via a "Copy raw log" button — for diagnosing an unmatched Lefu firmware layout. |
...
| `POST /api/scale/frames` (body: `user`, `session_id`, `adapter`, `frames` — raw BLE notification payloads as hex) | Folds one batch of a scan's frames into a running decode via the named adapter. Untrusted input (unknown-firmware scale, flaky connection) — sessions are buffered server-side, capped, and TTL-evicted. Returns `{state: "waiting"\|"reading"\|"final", ...}`; a `"final"` response includes `reading_id`, `weight_kg`, `impedance_ohm`, computed `metrics`, and persists the reading to `scale_readings`. | — |
